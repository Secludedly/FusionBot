...
#pragma warning restore CS0414                            // Flag to indicate if the form is still loading
        ...
        private Dictionary<Button, Timer> pulseTimers = new();
        ...
        private void InitializeFonts()
        {
            try
            {
                ...
                try
                {
                    lblTitleChildForm.Font = FontManager.Get("Gnuolane Rg", 20F);
                }
                catch
                {
                    lblTitleChildForm.Font = new Font(FontFamily.GenericSansSerif, 20F);
                }
            }
            catch (Exception ex)
            {
        ...
        // Update handling — disabled; code kept for reference only.
        private async void Updater_Click(object? sender, EventArgs e)
        {
            await Task.CompletedTask; // Update functionality is disabled.
            return;
        ...
        // Smooth hover fade
        private void StartHoverFade(Button btn, Color targetColor, int durationMs = 200)
        {
        ...
        private void StartOutlinePulse(Button btn, Color baseColor)
        {
        ...
        private void StopOutlinePulse(Button btn)
        {
        ...
        private async void FlashClick(Button btn)
        {
        ...
        private void InitializeFontsLink()
        {
            try
            {
                ...
                downloadFontsLink = new LinkLabel
                {
                    ...
                    BackColor = panelTitleBar.BackColor,
                    Cursor = Cursors.Hand
                };

                // Anchor to the top-right of the title bar panel
                downloadFontsLink.Anchor = AnchorStyles.Top | AnchorStyles.Right;
                downloadFontsLink.Location = new Point(panelTitleBar.ClientSize.Width - 120, 22);
