...
public partial class BotServer(Main mainForm, int port = 8080, int tcpPort = 8081) : IDisposable
{
    ...
    {
        // Update functionality is disabled — code kept for reference only.
        await Task.CompletedTask;
        return JsonSerializer.Serialize(new { success = false, message = "Update functionality is disabled." }, JsonOptions);
    ...
    private static async Task<string> CheckForUpdates()
    {
        // Update functionality is disabled — code kept for reference only.
        await Task.CompletedTask;
        return JsonSerializer.Serialize(new UpdateCheckResponse { Version = SysBot.Pokemon.Helpers.TradeBot.Version, Changelog = string.Empty, Available = false }, JsonOptions);
    ...
    {
        // Update functionality is disabled — code kept for reference only.
        return JsonSerializer.Serialize(new { success = true, message = "Update functionality is disabled.", action = "noop" }, JsonOptions);
